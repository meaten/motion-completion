...
parser = argparse.ArgumentParser(
  add_help=True)
parser.add_argument('--representation','-r', choices=['relative', 'absolute'])
args = parser.parse_args()
print(args)


def relative_coordinates( angles, parent, offset, rotInd, expmapInd):
  ...
  for i in np.arange( njoints ):
    ...
    if parent[i] == -1: # Root node
      ...
      xyzStruct[i]['xyz'] -= xyzStruct[ parent[i] ]['xyz']
...
def fkl( angles, parent, offset, rotInd, expmapInd ):
  """
  Convert joint angles and bone lengths into the 3d points of a person.
...
def main():
  ...
  train_data, complete_data = data_utils.load_data(data_dir, subjects, actions, True)
  ...
  if args.representation == 'absolute':
    for subject in subjects:
      for action in actions:
        for subact in [1, 2]:
          expmap_all = train_data[(subject, action, subact, 'even')]
          expmap_all = revert_coordinate_space(expmap_all, np.eye(3), np.zeros(3))
          pos = np.zeros([len(expmap_all), 96])
          for i, frame in enumerate(expmap_all):
            pos[i] = fkl(frame[:99], parent, offset, rotInd, expmapInd )
          np.save('./data/h3.6m/dataset_abs3d/S{0}/{1}_{2}.npy'.format(subject,
                                                                       action,
                                                                       subact), pos)
  if args.representation == 'relative':
    for subject in subjects:
      for action in actions:
        for subact in [1, 2]:
          expmap_all = train_data[(subject, action, subact, 'even')]
          ...
          pos = np.zeros([len(expmap_all), 96])
          for i, frame in enumerate(expmap_all):
            pos[i] = relative_coordinates(frame[:99], parent, offset, rotInd, expmapInd )
          np.save('./data/h3.6m/dataset_rel3d/S{0}/{1}_{2}.npy'.format(subject,
                                                                       action,
                                                                       subact), pos)
          print('data saved to ./data/h3.6m/dataset_rel3d/S{0}/{1}_{2}.npy'.format(subject,
                                                                                   action,
                                                                                   subact))
    ...
    for subject in subjects:
      for action in actions:
        for subact in [1, 2]:
          ...
          ob = viz.Ax3DPose(ax)
          for frame in expmap_all:
            ob.update(fkl(frame[:99], parent, offset, rotInd, expmapInd ))
    ...
    for subject in subjects:
      for action in actions:
        for subact in [1, 2]:
          ...
          ob = viz.Ax3DPose(ax)
          for frame in expmap_all:
            ob.update(fkl_relative(relative_coordinates(frame[:99], parent, offset, rotInd, expmapInd ),parent))
            plt.show(block=False)
            fig.canvas.draw()
            plt.pause(0.01)
  '''
